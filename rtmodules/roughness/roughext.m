function dataOut = roughext(dataIn,Fs)
% Calculates the roughness of a 8192 sample block of signal which has been 
% windowed with a blackman window. 
%
%
...
% FileIO is slower, but can access cached records like this
%initmatloadstring = strcat('InitFs',num2str(Fs),'N',num2str(N));
%load(initmatloadstring)
...
leveldB = 75; % Was 80 - I adjusted to give roughness = 1 for reference signal;
...
Cal	 	=	0.25; % calibration factor 
% - this is only applied to the Roughnesses. designed to compensate for signal level.
q		=	1:1:N; %indices up to number of FFT points
qb		=	N0:1:Ntop; 
freqs	=	(qb+1)*Fs/N;
hBPi	=	zeros(Chno,N);
hBPrms	=	zeros(1,Chno);
...
LdB		=	amp2db(Lg);      % convert to dB (20*log10(Lg))
...
for k=1:1:sizL %loop over freq indices above threshold
  ...
  for l = 1:1:whichZ(1,k) % loop up to floored bark number of freq index k
    ...
    if Stemp>MinBf(l) % provided this excitation is above the threshold 
      Slopes(k,l)=db2amp(Stemp); % critical filterbank lower side
    end
  end
  for l = whichZ(2,k):1:47 % loop up to ceil'd bark number 
    Stemp =	(S2(k)*((l*0.5)-Btmp))+Ltmp;
    if Stemp>MinBf(l)
      Slopes(k,l)=db2amp(Stemp); % critical filterbank upper side 
    end
  end
end
...
% Return the SPL of the signal input to it
SPL = rms(dataIn); % was mean(rms(dataIn)), not sure why.
if SPL > 0 
  %amp2dB just does 20*log10(x). 
  SPL = amp2db(SPL)+83; % -20 dBFS <--> 60 dB SPL  - investigate this! 
  % reports SPL of original signal pre-windowing
else
  SPL = -400;
end
