...
comp=[-50,20]; % compressor settings
...
initialize(mymodel);
c=0;
aamp=0:5:100;  % Ampitudes in dB SPL
for i=1:length(aamp)
    setvalue(p,aamp(i));
    run_once(mymodel);
    %     c=c+1; res(c)=0;
...
plot(aamp,res,'o-b');
set(gca,'ylim',[-5 105])
title('Input/Output function of simple hearing aid at 1000 Hz channel')
xlabel('input amplitude (dB SPL)')
ylabel('output amplitude (dB SPL)')
