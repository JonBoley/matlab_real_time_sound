...
classdef rt_model < handle   % derived from handle, so that every instance is just a pointer, not the full object
    ...
    methods
        function obj=rt_model(varargin)
            obj.parent=[]; % must be set later!
            obj.myname='real time sound model'; % this is me, now going to fill it
            ...
            obj.p=parameterbag('real time sound',obj);
            add(obj.p,param_int('SampleRate',pars.Results.SampleRate));
            add(obj.p,param_int('FrameLength',pars.Results.FrameLength));
            add(obj.p,param_int('Channels',pars.Results.Channels));
            add(obj.p,param_float('Duration',pars.Results.Duration));
            add(obj.p,param_checkbox('OverlapAdd',pars.Results.OverlapAdd));
            add(obj.p,param_float('PlotWidth',pars.Results.PlotWidth,'unittype',unit_time,'unit','sec'));
            
            % everything below here are the non-critical params, that can
            % be changed during run-time (and are if you are using
            % rt_full_gui
            
            obj.SampleRate=getvalue(obj.p,'SampleRate');
            obj.FrameLength=getvalue(obj.p,'FrameLength');
            obj.Channels=getvalue(obj.p,'Channels');
            obj.OverlapAdd=getvalue(obj.p,'OverlapAdd');
            obj.PlotWidth=getvalue(obj.p,'PlotWidth');
        end
        
        function obj=initialize(obj) %setvalue up everything and initialzise
            for i=1:length(obj.processes)
                initialize(obj.processes{i});
                % add a button to change parameter for each module
                %                 buttex=sprintf('%s',obj.processes{i}.basic_module.fullname);
                %                 butcalfct=sprintf('%s',obj.processes{i}.basic_module.full_name);
                %                 obj.p=add(param_button,'change params for','button_text',buttex,'button_callback_function',butcalfct);
            end
        end
        
        % find the process that contains this module:
        function  ret=getprocess(obj,mod)
            ret=[];
            for i=1:length(obj.processes)
                if isequal(obj.processes{i}.basic_module,mod)
                    ret=obj.processes{i};
                    return
                end
            end
        ...
        function replace_process(obj,old,new) % replace the old process with the new process, where the 'new' process is already in the cascade
            ...
            for i=1:length(obj.processes)
                a=obj.processes{i};
                if isequal(a,old)
                    close(obj.processes{i});
                    obj.processes{i}=new;
                    break;
                end
            end
            % and delete the new one from the list too:
            nr=length(obj.processes);
            if isequal(obj.processes{nr},new) % sanity check
                for i=1:nr-1
                    newprocesses{i}=obj.processes{i};
                end
                obj.processes=newprocesses;
            end
        end
        ...
        function run(obj,duration)
            ...
            obj.frame_counter=0;
            while obj.IsRunning
                obj.frame_counter=obj.frame_counter+1; % and reset global counters
                ...
                obj.global_time=obj.global_time+obj.FrameLength/obj.SampleRate;
                if obj.global_time>duration
                    obj.IsRunning=0;
                    break
                end
                drawnow limitrate
        ...
        function run_once(model)
            % for file input: run exactly as long as the file is
            %             inp_pro=model.processes{1}.basic_module;
            %             duration=get_file_duration(inp_pro);
...
%                     calib=20*log10(maxamp/1); % how many more dB because of pascale
        ...
        function register_window(obj,f)
            nr=length(obj.all_windows);
            obj.all_windows(nr+1)=f;
        end
