function [pesq_mos]= my_pesq(ref_data, deg_data,fs)
%% edited by Stefan Bleeck 2017 for easier use
% LoizouPesq must be in the path
...
sampling_rate=16000;
if fs~=16000
%     p1=audioplayer(ref_data,fs);play(p1);
    ref_data=resample(ref_data,sampling_rate,fs);
%     p2=audioplayer(ref_data,sampling_rate);play(p2);
    deg_data=resample(deg_data,sampling_rate,fs);
end

% [ref_data,sampling_rate,nbits]= wavread( ref_data);
...
%    Window(1+ count) = 0.5 * (1.0 - cos((2*pi * count) / Align_Nfft));
%end

count=0:Align_Nfft- 1;
Window= 0.5 * (1.0 - cos((2*pi * count) / Align_Nfft));
